...
- **ChatGPT travado (2ª rodada)**: causa 1 — layout mobile abaixo de 768 px dispara login direto que aborta → largura mínima
  780 px e aviso de zoom; causa 2 — Cloudflare/OpenAI retém a conexão sem responder (intermitente) → aviso após 12 s com
  Tentar novamente / Abrir no navegador. `navigationRequested` deixou de bloquear `about:blank` em iframes. Detalhes em 02 B16.
